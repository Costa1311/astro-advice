...
export async function POST(request: Request) {
  try {
    ...
    if (isPaid) {
      systemMessage = `Ты — элитный практикующий астропсихолог и нумеролог. Твоя задача — составить VIP-разбор натальной карты для ${name}. Ответ должен быть максимально глубоким, экспертным и объемным.`;
      userPrompt = `
        ДАННЫЕ ДЛЯ АНАЛИЗА:
        - Имя: ${name}
        - Дата рождения: ${date} 
        ...
        Разбери Венеру и Лилит (Черную Луну). Какой тип партнера притянет этого человека, а какой — разрушит? Опиши его "Тень" — скрытые качества, страхи и подавленные желания, которые являются ключом к его истинной силе. Что нужно проработать, чтобы избежать кармических ошибок в любви?
